...
async function getDataFromAoe2(steeam_id) {
  let data = await getAoe2DEMatchHistory(steeam_id);
  console.log(data);
  updateplayerDatatoDOM(data);
}

function updateplayerDatatoDOM(data) {
...
async function getAoe2DEMatchHistory(steeam_id) {
  let aoe2netUrl = new URL(`https://cors-anywhere.herokuapp.com/https://aoe2.net/api/player/ratinghistory`);
